...
function initResponsiveTable(tableSelector, desktopDataTableOptions, mobileColOrder) {
  ...
  if (mobileColOrder !== undefined) {
    mobileOptions.colReorder = {"order": mobileColOrder, "fixedColumnsLeft": 99};
  }
  ...
  function mobileToDesktopCallback() {
    table.colReorder && table.colReorder.reset();
...
function constrainName(data, type) {
  if (!MOBILE.isMobile() || type !== 'display') {
    return data;
  }
  var MAX_CHARS = 14;
  var $div = $('<div>');
  $div.html(data);
  var $nameContainer = $div.find('.currency-name-container'), name = $nameContainer.html();
  name = name.length > MAX_CHARS ? name.substr(0, MAX_CHARS - 2) + '...' : name;
...
function getLanguageOptions() {
  return {
    ...
    aria: {sortAscending: polyglot.t('sortAscending'), sortDescending: polyglot.t('sortDescending')}
  }
}
...
$(document).ready(function () {
  ...
  var currencyTableOptions = {
    ...
    "bPaginate": false,
    "bStateSave": false,
    "aoColumnDefs": [{"aTargets": [1], "render": constrainName}, {
      "bSortable": false,
      "aTargets": [7]
  ...
  var tables = $('#currencies-all, #assets-all').dataTable({
    ...
    "bPaginate": false,
    "bStateSave": false,
    "aoColumnDefs": [{"asSorting": ["desc", "asc"], "aTargets": [3, 4, 5, 6, 7, 8, 9]}],
    "sDom": 't',
    "language": langOptions
  });
  if (tables.length) {
  ...
  $('#markets-table').dataTable({
    ...
    "bPaginate": false,
    "bStateSave": false,
    "aoColumnDefs": [{"asSorting": ["desc", "asc"], "aTargets": [3, 4, 5]}],
    "language": langOptions
  });
  ...
  $('#trending-recently-added').DataTable({
    ...
    "bStateSave": false,
    "order": [],
    "aoColumnDefs": [{"asSorting": ["desc", "asc"], "aTargets": [3, 4, 5, 6, 7]}],
    "language": langOptions
  });
...
function toggle_currency(currency, currencyId) {
  ...
  $.each($('.percent-change'), function (key, value) {
    var elem = $(this);
    symbol = $(this).data("symbol").toString();
    if (non_fiat_currencies.indexOf(currency_lowercase) == -1) {
      percentage_change = elem.data("percentusd");
    }
    else if (symbol.toLowerCase() == currency_lowercase) {
      percentage_change = 0;
    } else {
...
function rerenderCurrency() {
  ...
  renderCurrency({currency: currency, currencyid: currencyid});
}
...
$(document).ready(function () {
  ...
  $(".price-toggle").on('click', function () {
    ...
    var label = $(this).html();
    renderCurrency({currency: currency, currencyid: currencyid, label: label});
    dataLayer.push({
      'event': 'customEvent',
...
HighChartsGraph.prototype.bindLogScaleToggle = function (linearToggleElem, logarithmicToggleElem) {
  var that = this;
  var chart = $('#' + that.graphId).highcharts();
  linearToggleElem.on('click', function () {
    chart.showLoading("Updating...");
    chart.update({
      yAxis: [{type: 'linear'}, {type: 'linear', floor: 0}, {
        type: 'linear',
        floor: 0
  ...
  logarithmicToggleElem.on('click', function () {
    ...
    chart.update({
      ...
      }, {type: 'logarithmic', floor: 'null', gridLineWidth: marketCapEnabled ? 0 : 1}, {
        type: 'logarithmic',
        floor: 'null',
        gridLineWidth: marketCapEnabled ? 0 : 1
      }, {type: 'logarithmic'}]
...
CurrencyDetailGraph.prototype.initCharts = function (seriesData) {
  ...
  Highcharts.setOptions({lang: defaultLangOptions(this.polyglot), global: {timezoneOffset: timezoneOffset}});
  $('#' + that.graphId).highcharts('StockChart', {
    chart: {backgroundColor: 'transparent', type: 'line', zoomType: 'x', ignoreHiddenSeries: true},
    responsive: {
      rules: [{
        ...
        chartOptions: {
          ...
          legend: {itemDistance: 30, itemMarginBottom: 15, itemStyle: {fontSize: '15px'}},
          scrollbar: {enabled: false},
          yAxis: [{title: {enabled: false}, labels: {align: 'left', x: 0}}, {
            title: {enabled: false},
            labels: {align: 'right', x: 0}
          }, {title: {enabled: false}, labels: {align: 'right', x: 0, y: 10}}, {
            title: {enabled: false},
            labels: {align: 'right', x: 0, y: 20}
          }, {title: {enabled: false}, labels: {align: 'left', x: 0}}]
        }
      }]
    },
    tooltip: {
    ...
    legend: {
      ...
      itemHoverStyle: {color: '#2a6496'},
      itemHiddenStyle: {color: '#757575'}
    },
    navigator: {adaptToUpdatedData: false, series: [{data: seriesData["market_cap_by_available_supply"]}]},
    scrollbar: {liveRedraw: false},
    title: {text: this.polyglot.t('chartTitle'), align: "left", style: {color: '#757575', fontSize: "24px"}},
    subtitle: {text: ''},
    ...
    yAxis: [{
      labels: {
        ...
        }, align: 'right', style: {color: '#56b4e9'}
      },
      title: {text: this.polyglot.t('marketCap'), style: {color: '#56b4e9', 'font-weight': 'bold'}},
      showEmpty: false,
      height: '80%',
      opposite: false,
      floor: 0,
    }, {
      labels: {formatter: label_format_fiat, style: {color: '#009e73',}, align: "left", x: 15},
      title: {text: this.polyglot.t('priceUSD'), style: {color: '#009e73', 'font-weight': 'bold'}},
      showEmpty: false,
      height: '80%',
      opposite: true,
      floor: 0
    }, {
      labels: {formatter: label_format_btc, style: {color: '#ff9f00',}, align: "left", x: 15},
      title: {text: this.polyglot.t('priceBTC'), style: {color: '#ff9f00', 'font-weight': 'bold'}},
      showEmpty: false,
      height: '80%',
      opposite: true,
      floor: 0
    }, {
      labels: {formatter: label_format_crypto_platform, style: {color: '#d55e00',}, align: "left", x: 15},
      title: {text: this.polyglot.t('pricePlatform'), style: {color: '#d55e00', 'font-weight': 'bold'}},
      showEmpty: false,
      height: '80%',
      opposite: true,
      floor: 0
