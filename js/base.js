  ...
  templates['quick_search'] = template({
    "compiler": [6, ">= 2.0.0-beta.1"], "main": function (depth0, helpers, partials, data) {
      ...
      return "<a href=\""
        ...
        + ".png\" class=\"currency-logo-sprite\" /> "
...
function initQuickSearch(options, polyglot) {
  ...
  var engine = new Bloodhound({
    name: 'currencies',
    prefetch: {'url': currencyDataURL, 'ttl': 2},
    limit: currencyLimit,
  ...
  var exchanges = new Bloodhound({
    name: 'exchanges',
    prefetch: {'url': exchangeDataURL, 'ttl': 2},
    limit: exchangeLimit,
    datumTokenizer: function (d) {
      return Bloodhound.tokenizers.whitespace(d.tokens.join(' '));
    },
  ...
  $('.js-quick-search').typeahead({hint: false, highlight: false, minLength: 1, autoselect: true}, {
    name: 'currencies',
    displayKey: 'name',
...
var Formatter = (function () {
  ...
  return {toLocaleString: toLocaleString};
})();
...
function AbbreviatedNumber(value, decPlaces) {
  ...
  var number = (function (number, decPlaces) {
    ...
    return {value: number, abbreviation: abbreviation};
  })(value, decPlaces);

  function _toLocaleString() {
    var val = Formatter.toLocaleString(number.value, {minDecimalPlaces: 2, maxDecimalPlaces: 2});
...
function convert_percent(elem, amount) {
  ...
  amount = Formatter.toLocaleString(amount, {minDecimalPlaces: 2, maxDecimalPlaces: 2});
  elem.html(amount + "%")
}
...
function format_percentage(val) {
  if (val > 9000) {
    ...
    val = Formatter.toLocaleString(val, {minDecimalPlaces: 2, maxDecimalPlaces: 2});
  }
  return val;
}
...
function format_supply(val) {
  if (val >= 1) {
    ...
    val = Formatter.toLocaleString(val, {minDecimalPlaces: 8, maxDecimalPlaces: 8});
  }
  return val;
}
...
function format_fiat(val) {
  if (val >= 100000 || val == 0) {
    ...
    val = Formatter.toLocaleString(val, {minDecimalPlaces: 6, maxDecimalPlaces: 6});
  }
  return val;
}
...
function format_crypto_helper(val, expenonentialThreshold, exponentialDigits, minDecimalPlaces, maxDecimalPlaces) {
  if (val >= 1000 || val == 0) {
    ...
    val = Formatter.toLocaleString(val, {minDecimalPlaces: minDecimalPlaces, maxDecimalPlaces: maxDecimalPlaces});
  } else {
...
function renderCurrencyDetailFields(currency) {
  ...
  $.each([$('[data-currency-market-cap]'), $('[data-currency-price]'), $('[data-currency-volume]')], function () {
...
function runNumberFormatters() {
  ...
  for (var selector in formatters) {
    ...
    $(selector).each(function () {
      var $this = $(this), value = $this.data('format-value');
...
var Currency = (function () {
  var containerSelector = '[data-global-currency-switch]', displaySelector = '[data-currency-display]',
    toggleSelector = '[data-currency-toggle]', changeEvent = 'currencyChange', currentValue = null;
  var cookie = {
    get: function () {
      return Cookies.get('currency');
    }, set: function (value) {
      var daysTilExpire = 3650;
      Cookies.set('currency', value, {expires: daysTilExpire});
    }
  };
...
(function () {
  ...
  $.ajax({
    ...
    success: function (data) {
      $('[data-global-stats-cryptocurrencies]').html(data.active_cryptocurrencies);
      $('[data-global-stats-markets]').html(data.active_markets);
      $('[data-global-stats-market-cap]').data('usd', data.total_market_cap_by_available_supply_usd);
      $('[data-global-stats-volume]').data('usd', data.total_volume_usd);
      $('[data-global-stats-btc-dominance]').html(data.bitcoin_percentage_of_market_cap.toFixed(1));
      $('[data-global-stats-container]').removeClass('hidden');
...
(function ($) {
  ...
  function toggleMobileText(context) {
    const DATA_ATTR_SELECTOR = '[data-mobile-text]', DATA_ATTR_NAME = 'mobile-text';
    var selection = context === undefined ? $(DATA_ATTR_SELECTOR) : $(DATA_ATTR_SELECTOR, context);
    selection.each(function (index, element) {
      var $element = $(element), targetText = $element.data(DATA_ATTR_NAME), sourceText = $element.html();
  ...
  window.MOBILE = {
    isMobile: function () {
      return isMobile;
    }, toggleMobileText: toggleMobileText, onBrowserResize: onBrowserResize
  };
})($);